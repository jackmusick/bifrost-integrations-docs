...
```python
from bifrost import workflow, param, oauth

@workflow(category="User Management")
@param("department", data_provider="get_departments")  # Dropdown from data provider
...
The decorator automatically infers name (`"onboard_user"`) and description from the docstring. Parameters are extracted from the function signature with proper types.

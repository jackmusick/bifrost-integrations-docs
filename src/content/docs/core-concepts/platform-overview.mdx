...
<Card title="Multi-Tenant Architecture" icon="users">
  ...
  <Card title="AI Agents" icon="rocket">
    - Conversational agents with tool-calling workflows
    - Autonomous agent runs triggered by events or schedules
    - Budget controls for iteration and token limits
    - Full observability with step-by-step run tracking
  </Card>
...
AI agents that use workflows as tools:

- **Chat mode**: Interactive conversations via web, Teams, or Slack
- **Autonomous mode**: Headless execution triggered by events, schedules, or SDK calls
- Budget controls (`max_iterations`, `max_token_budget`) bound resource usage

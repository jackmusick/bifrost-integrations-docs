...
Workflows are Python functions decorators to describe functionality to the platform. The Workflows page shows all available workflows organized by category.

![Workflows List](../../../assets/core-concepts/workflows.png)
...
1. Developer registers the function via Code Editor, API, or MCP
...
Workflows require explicit registration after creating the file. Write your code, then register the function using the Code Editor, the `POST /api/workflows/register` endpoint, or the `register_workflow` MCP tool.
...
See [Workflow Registration](/core-concepts/discovery-system) for details on the registration process.

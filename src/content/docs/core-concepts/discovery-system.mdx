...
### CodeLens in the Code Editor (Recommended)
...
<Aside type="tip">
CodeLens registration also handles **reactivation**. If you previously deleted a workflow and re-register the same function, the platform reuses the original UUID so that any references (event subscriptions, form bindings) still work.
</Aside>

...
styles.css            # Custom CSS (dark mode via .dark selector, supports @apply / @layer)
tailwind.config.ts    # Optional per-app Tailwind theme overrides
pages/
  index.tsx           # Home page (/)
...
Apps run a full per-app Tailwind v4 pipeline. The following all work:

| Feature | Example |
|---------|---------|
| Arbitrary values | `max-w-[1400px]`, `min-h-[calc(100vh-4rem)]`, `px-[clamp(1rem,3vw,2.5rem)]` |
| `var()` in classes | `bg-[color:var(--app-surface)]`, `bg-[hsl(var(--accent)/0.6)]` |
| Modern color spaces | `bg-[oklch(0.5_0.18_260)]` |
| Complex grids | `lg:grid-cols-[minmax(0,1fr)_360px]`, `grid-cols-[repeat(auto-fit,minmax(240px,1fr))]` |
| `@apply` in `styles.css` | `.btn-primary { @apply bg-primary text-primary-foreground rounded-md px-4 py-2; }` |
| `@layer components` | `@layer components { .card-soft { @apply rounded-2xl border bg-card/60 p-6; } }` |
| `:root` and `.dark` variables | as shown above |
| Per-app `tailwind.config.{ts,js,mjs,cjs}` | extends theme tokens (colors, fonts, spacing) for that app only |

Drop a `tailwind.config.ts` next to `app.yaml` to extend tokens scoped to your app. No platform restart needed -- the bundler picks it up on the next preview build.

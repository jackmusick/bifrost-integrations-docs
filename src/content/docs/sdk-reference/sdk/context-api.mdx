...
<Aside type="note">
The `context` proxy provides identity, organization, and execution metadata. It's automatically available during workflow execution. The only mutation allowed is `set_scope()` for cross-org access.
</Aside>
...
:::note
To read configuration values, use `bifrost.config.get()` instead of `context.get_config()` (which has been removed).
:::
...
<Aside type="caution">
Only provider organizations can call `set_scope()` with a different org ID. Non-provider orgs will receive a `PermissionError`.
</Aside>

...
<Aside type="note">
Settings like `timeout_seconds`, `schedule`, `endpoint_enabled`, `allowed_methods`, `public_endpoint`, and `execution_mode` are configured via the Bifrost UI or API, not in the decorator.
</Aside>
...
| `list(org_id, include_inactive)` | org_id: str \| None, include_inactive: bool = False | list[UserPublic] | List users |
| `update(user_id, **updates)` | user_id: str, updates: dict | UserPublic | Update user |
| `delete(user_id)` | user_id: str | bool | Permanently delete user |
...
# Include inactive (disabled) users
all_users = await users.list(org_id="org-123", include_inactive=True)
```
...
Workflow metadata, execution status, and fire-and-forget execution.
...
| `get(execution_id)` | execution_id: str | WorkflowExecution | Get execution details |
| `execute(workflow_name, params)` | workflow_name: str, params: dict | str | Execute a workflow asynchronously, returns execution ID |
...
# Returns immediately with execution ID — does not wait for completion
```

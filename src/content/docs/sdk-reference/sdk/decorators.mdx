...
<Aside type="note">
Decorators are applied **bottom-to-top**. This means the decorator closest to the function is applied first, and the decorator at the top is applied last.
</Aside>
...
```
  ...
  <TabItem label="Scheduled" icon="clock">
```python
@workflow(
    name="daily_report",
...
```
  ...
  <TabItem label="With Retry" icon="refresh">
```python
@workflow(
...
```
  ...
  <TabItem label="HTTP Endpoint" icon="link">
```python
@workflow(
    name="handle_webhook",
    description="Handle incoming webhook",
    endpoint_enabled=True,
    allowed_methods=["POST"],
    public_endpoint=True  # No auth required
)
async def handle_webhook(context, payload: dict):
    # Implementation
    pass
```
  </TabItem>
...
```
  ...
  <TabItem label="Select" icon="list">
```python
@param(
    "department",
    "select",
...
```
  ...
  <TabItem label="Number" icon="hashtag">
```python
@param(
    "user_count",
    "number",
...
```
  ...
  <TabItem label="Boolean" icon="approve-check">
```python
@param(
    "notify_admin",
    "boolean",
...
```
  ...
  <TabItem label="Multi-Select" icon="list">
```python
@param(
    "permissions",
    "multi_select",
...
```
  ...
  <TabItem label="Error Handling" icon="warning">
```python
import logging
...
<Aside type="tip">
Data providers with `cache_ttl_seconds > 0` are cached. Results are reused until the cache expires, reducing API calls and improving form load times.
</Aside>

## Decorator Stacking

<Aside type="caution">
Multiple decorators can be stacked. Decorators are applied **bottom-to-top** - the decorator closest to the function is applied first.
</Aside>
...
<Aside type="tip">
Follow these guidelines for production-ready workflows and data providers:
</Aside>

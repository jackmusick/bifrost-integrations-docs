...
| `scope` | str \| None | Omit for context org (cascade), `None` for global, UUID for specific org (provider only) |
...
# Create global table (scope=None targets global directly)
shared = await tables.create("shared_lookups", scope=None)
```
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `scope` | str \| None | Omit for context org (includes global via cascade), `None` for global only, UUID for specific org |
...
Tables with `scope=None` are platform-wide:

```python
# Global table (no org_id)
table = await tables.create("shared_lookups", scope=None)
```

...
**Returns** to workflow (single file):
...
```json
{
    ...
    "size": 1024000,
    "path": "form-id/uuid/document.pdf"
}
```

**Access in workflow** using the files SDK:

```python
from bifrost import files

async def process_upload(context, document):
    # Read the uploaded file content
    content = await files.read(document["path"], location="uploads")

    # Or get file info
    info = await files.info(document["path"], location="uploads")
    print(f"File size: {info.size} bytes")
```

For multiple files, the field returns an array of file metadata objects.
